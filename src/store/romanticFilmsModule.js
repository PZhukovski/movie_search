...
export const romanticFilmsModule = {
  state: () => ({
    romanticFilms: [],
  ...
  mutations: {
    setFilms(state, romanticFilms) {
      state.romanticFilms = romanticFilms;
    },
  ...
  actions: {
    async fetchRomanticFilms({ state, commit }) {
      try {
  ...
  getters: {
    getRomanticFilms(state) {
      return state.romanticFilms;
    },
  },
  namespaced: true,
};
