...
export const cannesFilmsModule = {
  state: () => ({
    cannesFilms: [],
  ...
  mutations: {
    setFilms(state, cannesFilms) {
      state.cannesFilms = cannesFilms;
    },
  ...
  actions: {
    async fetchCannesFilms({ state, commit }) {
      try {
  ...
  getters: {
    getCannesFilms(state) {
      return state.cannesFilms;
    },
  },
  namespaced: true,
};
